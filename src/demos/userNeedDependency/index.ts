 ...
 * - `phase1.ts` — personalize the value chain via a 5-step form.
 * - `phase2.ts` — turn the value chain into a Wardley Map by placing every node on the evolution axis.
 * - `phase3.ts` — think with the map: one doctrine question per capability, each answer anchored to the map.
