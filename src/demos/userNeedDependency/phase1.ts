...
/** the mascot's Phase 1 bubble copy — shown at each "waiting for Next" pause around the form */
...
/**
 * Phase 1: personalize the value chain. Shows a short "Nice!" placeholder while the visitor
 * clicks the "Next" link (the Phase 0 -> Phase 1 gate) rendered into `nextControl`, then walks
 * the visitor through a 5-step form (`Mascot.showField`: need -> user -> 3 capabilities),
 * re-anchoring to whichever node each question is about before asking it, relabeling each
 * placeholder node (both the domain `ValueChain` in `ctx.chain` and the rendered canvas node) as
 * its answer comes in. Once personalized, shows an "All done!" placeholder and the chain
 * celebrates, firing `onCelebrate` — the caller then waits on a second "Next" link that gates the
 * move into Phase 2 (`phase2.ts`).
 */
export async function runPhase1(ctx: ScenarioContext): Promise<void> {
